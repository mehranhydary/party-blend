    ...
    UUPSUpgradeable
{
    ...
    function execute(
        Input calldata sell,
        Input calldata buy
    ) external payable whenOpen setupExecution {
        _execute(sell, buy);
        _returnDust();
    }
 ...
 */
        ...
        for (uint8 i = 0; i < executionsLength; i++) {
            assembly {
                ...
                let order_location := calldataload(
    ...
    function _validateSignatures(
        ...
        if (
            order.order.extraParams.length > 0 &&
            order.order.extraParams[0] == 0x01
        ) {
    ...
    function _validateUserAuthorization(
        ...
        if (signatureVersion == SignatureVersion.Single) {
            ...
            bytes32[] memory merklePath = abi.decode(
                extraSignature,
                (bytes32[])
            );

            bytes32 computedRoot = MerkleVerifier._computeRoot(
                orderHash,
                merklePath
            );
