...
library CalculationHelpers {
    ...
    function computeCurrentDebt(
        ...
        return
            uint256(
                wadMul(
                    int256(amount),
                    wadExp(wadMul(yearsWad, bipsToSignedWads(rate)))
                )
            );
    }
    ...
    function calcRefinancingAuctionRate(
        ...
        int256 middleSlope = bipsToSignedWads(9000) /
            int256((3 * auctionDuration) / 5) +
            1; // wad-bips per block (add one to account for rounding)
